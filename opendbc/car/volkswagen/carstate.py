...
class CarState(CarStateBase, MadsCarState):
  ...
  def update_mqb(self, pt_cp, cam_cp, ext_cp) -> structs.CarState:
    ...
    self.parse_mlb_mqb_steering_state(ret, ret_ic, pt_cp)
  ...
  def update_meb(self, pt_cp, cam_cp, alt_cp, ext_cp) -> tuple[structs.CarState, structs.CarStateSP, structs.CarStateIC]:
    ...
    ret.carFaultedNonCritical = cam_cp.vl["EA_01"]["EA_Funktionsstatus"] in (3, 4, 5, 6) if self.CP.flags & VolkswagenFlags.STOCK_EA_PRESENT else False
  ...
  def parse_mlb_mqb_steering_state(self, ret, ret_ic, pt_cp, drive_mode=True):
    ret.steeringAngleDeg = pt_cp.vl["LWI_01"]["LWI_Lenkradwinkel"] * (1, -1)[int(pt_cp.vl["LWI_01"]["LWI_VZ_Lenkradwinkel"])]
    ret.steeringRateDeg = pt_cp.vl["LWI_01"]["LWI_Lenkradw_Geschw"] * (1, -1)[int(pt_cp.vl["LWI_01"]["LWI_VZ_Lenkradw_Geschw"])]
    ret.steeringTorque = pt_cp.vl["LH_EPS_03"]["EPS_Lenkmoment"] * (1, -1)[int(pt_cp.vl["LH_EPS_03"]["EPS_VZ_Lenkmoment"])]
    ret.steeringPressed = abs(ret.steeringTorque) > self.CCP.STEER_DRIVER_ALLOWANCE
